...
const clusterWorkerSize = os.cpus().length - 7;
...
//    console.log('Message Recieved: ' + message);
    ...
    const eventRef = {
        ...
        "App\\Modules\\WorkstationModule\\Events\\AgentWipeoutBroadcast" : 'agent:wipeout',
        "App\\Modules\\WorkstationModule\\Events\\AgentLockBroadcast" : 'agent:lockscreen',
        "App\\Modules\\WorkstationModule\\Events\\AgentScheduledWipeoutBroadcast" : 'agent:scheduled-wipeout',
    ...
    switch( message.event ){
        // PC Lock Request
        case "App\\Modules\\WorkstationModule\\Events\\AgentLockBroadcast":
            eventName  = 'agent:lockscreen';
...
//    console.log("EMIT : ", message.data );
	...
	if(eventName == 'agent:status-check') { return; }
...
http.listen(port, function(){
    console.log('node-dashboard: running on Port ' + port);
});
...
const start = async () => {
	try {
		if (socketHost) {
      await http.listen(port, socketHost, function () {
...
// additional config with nginx
http.keepAliveTimeout = 65 * 1000;
http.headersTimeout = 70 * 1000;
