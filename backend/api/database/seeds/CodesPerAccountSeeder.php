...
class CodesPerAccountSeeder extends Seeder
{
    /**
    ...
    public function run()
    {
        $json_data   = file_get_contents(resource_path() . '/seeds/codes_per_account.json');
        $insert_data = json_decode( $json_data );

        foreach( $insert_data as $row ){
            DB::table('codes_per_account')->insert( (array) $row );
        }
    }
}
